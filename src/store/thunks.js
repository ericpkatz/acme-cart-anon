...
import { consolidatedCart, loadOrdersFromLocal, addToLocalCart, removeFromLocalCart } from './localCart';
...
const exchangeTokenForAuth = (history)=> {
  return (dispatch)=> {
    ...
    .then(()=> {
      return dispatch(consolidateCart());
    })
    .catch( ex => {
...
const consolidateCart = ()=> {
  return (dispatch, getState)=> {
    const serverCart = getState().orders.find( order => order.status === 'CART');
    const changes = consolidatedCart(serverCart);
    return Promise.all(changes.map( change => dispatch(addToCart(serverCart, change.product, change.quantity, change.lineItem))));
  }
};
...
const addToCart = (cart, product, quantity, lineItem)=> {
  return (dispatch, getState)=> {
    ...
    if(userId){
      ...
      promise = addToLocalCart(cart, product, quantity, lineItem);
    }
    return promise
...
const removeFromCart = (cart, lineItem)=> {
  return (dispatch, getState)=> {
    ...
    if(userId){
      ...
      promise = removeFromLocalCart(cart, lineItem);
    }
    return promise
      .then( () => dispatch(loadOrders())); 
  };
